    ...
    <div className="space-y-8 animate-in fade-in zoom-in duration-500">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Đã Hết Hạn', value: '1', icon: XCircle, color: 'text-admin-text-muted', bg: 'bg-[#8A8F98]/10 border-[#8A8F98]/20' },
        ].map((s, i) => (
                  ...
                  <motion.tr key={c.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => setIsModalOpen(true)} className="p-2 rounded-lg bg-[#4F1F76]/20 text-admin-text-muted hover:text-[#00FFFF] hover:bg-[#00FFFF]/10 transition-all"><Edit2 className="w-4 h-4" /></button>
                        <button className="p-2 rounded-lg bg-[#4F1F76]/20 text-admin-text-muted hover:text-red-500 hover:bg-red-500/10 transition-all"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </motion.tr>
                );
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                <div>
                  ...
                  <input type="text" placeholder="VD: EARLY10, VIP30..." className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-admin-text font-mono focus:outline-none focus:border-[#FF0088]" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest block mb-2">Mô Tả</label>
                  <input type="text" placeholder="Mô tả ngắn gọn về chiến dịch..." className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-admin-text focus:outline-none focus:border-[#FF0088]" />
                </div>
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    ...
                    <select className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-admin-text focus:outline-none focus:border-[#FF0088] appearance-none">
                  ...
                  <div>
                    ...
                    <input type="number" placeholder="10" className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-admin-text focus:outline-none focus:border-[#FF0088]" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <label className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest block mb-2">Giới Hạn Lượt Dùng</label>
                    <input type="number" placeholder="500" className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-admin-text focus:outline-none focus:border-[#FF0088]" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest block mb-2">Ngày Hết Hạn</label>
                    <input type="date" className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-admin-text focus:outline-none focus:border-[#FF0088]" />
                  </div>
                </div>
              </div>
              <div className="p-6 border-t border-admin-border flex justify-end gap-4 shrink-0">
                ...
                <button className="px-8 py-3 rounded-xl bg-gradient-to-r from-[#FF0088] to-[#4F1F76] text-admin-text font-bold flex items-center gap-2 hover:scale-[1.02] transition-transform glow-magenta">
