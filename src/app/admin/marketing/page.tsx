...
export default function AdminMarketingPage() {
  ...
  const getTypeIcon = (t: string) => {
    if (t === 'PERCENT') return <Tag className="w-4 h-4 text-[#FF0088]" />;
    if (t === 'AMOUNT') return <Percent className="w-4 h-4 text-[#E6C753]" />;
  ...
  return (
    <div className="space-y-8 animate-in fade-in zoom-in duration-500">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Đang Chạy', value: campaigns.filter(c => c.status === 'ACTIVE').length.toString(), icon: Megaphone, color: 'text-[#FF0088]', bg: 'bg-[#FF0088]/10 border-[#FF0088]/20' },
          { label: 'Tổng Lượt Dùng Mã', value: campaigns.reduce((acc, c) => acc + c.used, 0).toLocaleString(), icon: Tag, color: 'text-[#00FFFF]', bg: 'bg-[#00FFFF]/10 border-[#00FFFF]/20' },
          { label: 'Hết Lượt', value: campaigns.filter(c => c.status === 'USED_UP').length.toString(), icon: Ban, color: 'text-[#E6C753]', bg: 'bg-[#E6C753]/10 border-[#E6C753]/20' },
          { label: 'Hết Hạn', value: campaigns.filter(c => c.status === 'EXPIRED').length.toString(), icon: XCircle, color: 'text-[#8A8F98]', bg: 'bg-[#8A8F98]/10 border-[#8A8F98]/20' },
        ].map((s, i) => (
      ...
      <div className="glass-card rounded-[2rem] bg-[#0D0716]/80 backdrop-blur-md border border-[#4F1F76]/30 overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-[#4F1F76]/30 bg-[#4F1F76]/5 flex items-center justify-between">
          <h3 className="text-lg font-display font-black text-white uppercase tracking-wider">Danh Sách Chiến Dịch</h3>
          <span className="text-xs text-[#8A8F98]">{campaigns.length} kết quả</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#4F1F76]/10 text-[#8A8F98] text-xs uppercase tracking-wider">
                <th className="p-4 font-bold border-b border-[#4F1F76]/30 min-w-[200px]">Chiến Dịch</th>
                ...
                <th className="p-4 font-bold border-b border-[#4F1F76]/30 hidden lg:table-cell">Ngày Mở</th>
                <th className="p-4 font-bold border-b border-[#4F1F76]/30 hidden lg:table-cell">Hết Hạn</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                return (
                  <motion.tr key={c.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                    ...
                    <td className="p-4">
                      <div className="font-mono font-black text-[#FF0088] text-sm">{c.code}</div>
                      <div className="text-xs text-[#8A8F98] mt-1 line-clamp-1">{c.name}</div>
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-2 text-sm text-[#8A8F98]">{getTypeIcon(c.type)} {c.type === 'PERCENT' ? 'Phần trăm' : 'Số tiền'}</div>
                    </td>
                    ...
                    <td className="p-4 min-w-[140px]">
                      <div className="text-[10px] text-[#8A8F98] mb-1.5 font-bold">{c.used}/{c.limit > 0 ? c.limit : '∞'} lượt</div>
                      <div className="h-2 rounded-full bg-white/5 overflow-hidden">
                        <div className="h-full rounded-full bg-gradient-to-r from-[#FF0088] to-[#00FFFF]" style={{ width: c.limit > 0 ? `${Math.min(pct, 100)}%` : '0%' }} />
                      </div>
                    </td>
                    ...
                    <td className="p-4 hidden lg:table-cell text-xs text-[#8A8F98] whitespace-nowrap">
                      <div className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {formatDate(c.startDate)}</div>
                    </td>
                    <td className="p-4 hidden lg:table-cell text-xs text-[#8A8F98] whitespace-nowrap">
          ...
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
              ...
              <form onSubmit={handleCreate} className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                <div>
                  <label className="text-[10px] font-bold text-[#8A8F98] uppercase tracking-widest block mb-2">Mã Giảm Giá *</label>
                  <input required type="text" value={formData.code} onChange={e => setFormData({...formData, code: e.target.value.toUpperCase()})} placeholder="VD: EARLY10, VIP30..." className="w-full bg-[#060010] border border-[#4F1F76]/50 rounded-xl px-4 py-3 text-[#FFFFFF] font-mono focus:outline-none focus:border-[#FF0088]" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-[#8A8F98] uppercase tracking-widest block mb-2">Tên Chiến Dịch / Mô tả</label>
                  <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="VD: Khuyến mãi mừng Noel..." className="w-full bg-[#060010] border border-[#4F1F76]/50 rounded-xl px-4 py-3 text-[#FFFFFF] focus:outline-none focus:border-[#FF0088]" />
                </div>
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <label className="text-[10px] font-bold text-[#8A8F98] uppercase tracking-widest block mb-2">Loại Giảm Giá</label>
                    <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full bg-[#060010] border border-[#4F1F76]/50 rounded-xl px-4 py-3 text-[#FFFFFF] focus:outline-none focus:border-[#FF0088] appearance-none">
                      <option value="PERCENT">Phần trăm (%)</option>
                      <option value="AMOUNT">Số tiền cố định (₫)</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[#8A8F98] uppercase tracking-widest block mb-2">Giá trị giảm</label>
                    <input required type="number" value={formData.value} onChange={e => setFormData({...formData, value: e.target.value})} className="w-full bg-[#060010] border border-[#4F1F76]/50 rounded-xl px-4 py-3 text-[#FFFFFF] focus:outline-none focus:border-[#FF0088]" />
                  </div>
                </div>
                <div>
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <label className="text-[10px] font-bold text-[#8A8F98] uppercase tracking-widest block mb-2">Ngày Mở Voucher</label>
                    <input type="date" value={formData.startDate} onChange={e => setFormData({...formData, startDate: e.target.value})} className="w-full bg-[#060010] border border-[#4F1F76]/50 rounded-xl px-4 py-3 text-[#FFFFFF] focus:outline-none focus:border-[#FF0088] [color-scheme:dark]" />
                    <p className="text-[9px] text-[#8A8F98]/60 mt-1">Để trống = có hiệu lực ngay</p>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[#8A8F98] uppercase tracking-widest block mb-2">Ngày Hết Hạn</label>
                    <input type="date" value={formData.endDate} onChange={e => setFormData({...formData, endDate: e.target.value})} className="w-full bg-[#060010] border border-[#4F1F76]/50 rounded-xl px-4 py-3 text-[#FFFFFF] focus:outline-none focus:border-[#FF0088] [color-scheme:dark]" />
                    <p className="text-[9px] text-[#8A8F98]/60 mt-1">Để trống = không hết hạn</p>
                  </div>
                </div>
                ...
                <div className="p-6 border-t border-[#4F1F76]/30 flex justify-end gap-4 shrink-0">
                  ...
                  <button type="submit" disabled={isSubmitting} className="px-8 py-3 rounded-xl bg-gradient-to-r from-[#FF0088] to-[#4F1F76] text-[#FFFFFF] font-bold flex items-center gap-2 hover:scale-[1.02] transition-transform glow-magenta disabled:opacity-50">
