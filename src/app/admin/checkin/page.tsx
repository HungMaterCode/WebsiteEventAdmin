...
export default function AdminCheckinPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in zoom-in duration-500">
      <div>
        <h2 className="text-3xl font-display font-black uppercase text-admin-text">Hệ Thống Check-In</h2>
        <p className="text-admin-text-muted text-sm mt-1">Quét mã QR, xác thực vé và kiểm soát lối vào theo thời gian thực</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Tổng Vé Bán', value: '1,250', color: 'text-admin-text', bg: 'bg-white/5 border-white/10', icon: Users },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* QR Scanner Panel */}
        <div className="lg:col-span-1 glass-card p-6 rounded-[2rem] bg-admin-panel/80 backdrop-blur-md border border-admin-border shadow-2xl">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-3 rounded-2xl bg-[#00FFFF]/10 border border-[#00FFFF]/20">
              <QrCode className="w-6 h-6 text-[#00FFFF]" />
            </div>
            <div>
              <h3 className="text-lg font-display font-black text-admin-text uppercase tracking-wider">Quét QR / Mã Vé</h3>
              <p className="text-[10px] text-admin-text-muted tracking-widest font-bold uppercase mt-0.5">Nhập thủ công hoặc dùng máy quét</p>
            </div>
          </div>

          {/* Scan Box */}
          <div className="aspect-square rounded-2xl bg-admin-bg border-2 border-dashed border-admin-border flex flex-col items-center justify-center mb-6 relative overflow-hidden group hover:border-[#00FFFF]/50 transition-colors">
            ...
            <p className="text-admin-text-muted text-xs font-bold uppercase tracking-widest">Khu Vực Quét QR</p>
        ...
        <div className="lg:col-span-2 glass-card rounded-[2rem] bg-admin-panel/80 backdrop-blur-md border border-admin-border overflow-hidden shadow-2xl">
          <div className="p-6 border-b border-admin-border bg-[#4F1F76]/5">
            <h3 className="text-lg font-display font-black text-admin-text uppercase tracking-wider">Log Check-In Thời Gian Thực</h3>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#4F1F76]/10 text-admin-text-muted text-xs uppercase tracking-wider">
                  <th className="p-4 font-bold border-b border-admin-border">Khách Hàng</th>
                  <th className="p-4 font-bold border-b border-admin-border hidden sm:table-cell">Hạng Vé</th>
                  <th className="p-4 font-bold border-b border-admin-border hidden md:table-cell">Khu Vực</th>
                  <th className="p-4 font-bold border-b border-admin-border text-center">Kết Quả</th>
                  <th className="p-4 font-bold border-b border-admin-border text-right">Giờ</th>
                </tr>
              </thead>
              <tbody>
                {mockCheckins.map((c, i) => {
                  ...
                  return (
                    <motion.tr key={c.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }}
                      className="border-b border-admin-border hover:bg-[#4F1F76]/5 transition-colors">
                      <td className="p-4">
                        <div className="font-bold text-admin-text text-sm">{c.guestName}</div>
                        <div className="text-xs text-admin-text-muted font-mono mt-0.5">{c.id}</div>
                      </td>
                      <td className="p-4 hidden sm:table-cell">
                        <span className={`text-xs font-black ${c.type === 'VIP' ? 'text-[#FF0088]' : c.type === 'EARLY BIRD' ? 'text-[#E6C753]' : 'text-admin-text'}`}>{c.type}</span>
                      </td>
                      <td className="p-4 hidden md:table-cell text-xs text-admin-text-muted">{c.zone}</td>
                      ...
                      <td className="p-4 text-right font-mono text-xs text-admin-text-muted">{c.checkedAt}</td>
                    </motion.tr>
                  );
                })}
              </tbody>
