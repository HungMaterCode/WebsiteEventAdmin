    ...
    <div className="space-y-8 animate-in fade-in zoom-in duration-500">
      ...
      <div className="glass-card rounded-[2rem] bg-admin-panel/80 backdrop-blur-md border border-admin-border overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-admin-border flex flex-col md:flex-row gap-4 justify-between items-center bg-[#4F1F76]/5">
          ...
          <button className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border border-admin-border text-admin-text-muted hover:text-admin-text hover:bg-[#4F1F76]/20 transition-all w-full md:w-auto">
                ...
                >
                  <td className="p-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-full border-2 border-admin-border p-0.5 shrink-0">
                        <div className="w-full h-full rounded-full bg-[#4F1F76]/30 flex items-center justify-center text-admin-text font-bold">
                          {customer.name.charAt(0)}
                        </div>
                      </div>
                      <div>
                        <div className="font-bold text-admin-text group-hover:text-[#00FFFF] transition-colors">{customer.name}</div>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => setIsModalOpen(true)} className="p-2 rounded-lg bg-[#4F1F76]/20 text-admin-text-muted hover:text-[#00FFFF] hover:bg-[#00FFFF]/10 transition-all" title="Chi tiết">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button className="p-2 rounded-lg bg-[#4F1F76]/20 text-admin-text-muted hover:text-red-500 hover:bg-red-500/10 transition-all" title="Cài đặt">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="relative w-full max-w-lg bg-admin-panel border border-admin-border rounded-[2rem] shadow-2xl overflow-hidden flex flex-col">
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <input type="text" defaultValue="Nguyễn Văn A" className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-admin-text focus:outline-none focus:border-[#FF0088]" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest block mb-2">Hạng Thành Viên</label>
                  <select className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-admin-text focus:outline-none focus:border-[#FF0088] appearance-none" defaultValue="VIP">
