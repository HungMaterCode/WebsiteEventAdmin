    ...
    <div className="space-y-8 animate-in fade-in zoom-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-admin-text-muted text-sm mt-1">Quản lý các câu hỏi thường gặp hiển thị trên trang chủ</p>
        </div>
        ...
        <Reorder.Group axis="y" values={items} onReorder={handleReorder} className="space-y-4">
          {items.map((item, index) => (
            <Reorder.Item
              ...
              className="glass-card p-5 md:p-6 rounded-3xl bg-admin-panel border border-admin-border flex flex-col md:flex-row items-start md:items-center gap-6 group hover:border-magenta/40 shadow-2xl relative overflow-hidden"
            >

              <div className="absolute top-0 left-0 w-1 h-full bg-magenta/50 group-hover:bg-magenta transition-colors" />

              <div className="flex items-center gap-5 shrink-0">
                <div className="text-admin-text-muted group-hover:text-magenta transition-colors cursor-grab active:cursor-grabbing p-2 hover:bg-admin-bg/5 rounded-lg">
              ...
              <motion.div className="flex-1 min-w-0" layout="position">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h4 className="text-lg font-bold text-admin-text uppercase tracking-wide line-clamp-1 group-hover:text-magenta transition-colors">{item.question}</h4>
                </div>
                <p className="text-admin-text-muted text-sm leading-relaxed line-clamp-2 pr-4">{item.answer}</p>
              </motion.div>
        ...
        {editingItem && (
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-admin-panel border border-admin-border rounded-[2.5rem] overflow-hidden shadow-[0_0_80px_rgba(255,0,136,0.15)]"
            >
              <div className="p-8 border-b border-admin-border flex justify-between items-center bg-admin-bg/50 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 relative z-10">
                  ...
                  <h3 className="text-xl font-display font-bold text-admin-text uppercase italic tracking-wider">{editingItem.id ? 'Sửa câu hỏi' : 'Thêm câu hỏi mới'}</h3>
                </div>
              ...
              <form onSubmit={handleAddEdit} className="p-8 space-y-6">
                <div className="space-y-5">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest block pl-1">Câu hỏi</label>
                    <input
                      required
                      type="text"
                      ...
                      className="w-full bg-admin-bg border border-admin-border rounded-2xl px-6 py-4 text-admin-text focus:outline-none focus:border-magenta font-bold outline-none transition-all placeholder:font-normal placeholder:opacity-30"
                    />
                  </div>

                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest block pl-1">Câu trả lời</label>
                    <textarea
                      required
                      ...
                      className="w-full bg-admin-bg border border-admin-border rounded-2xl px-6 py-4 text-admin-text focus:outline-none focus:border-magenta text-sm leading-relaxed outline-none transition-all resize-none shadow-[inset_0_0_20px_rgba(0,0,0,0.02)]"
                    />
                  </div>

                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest block pl-1">Phân loại (Tùy chọn)</label>
                    <div className="flex items-center gap-3">
                      <div className="p-3 rounded-xl bg-admin-bg border border-admin-border text-admin-text-muted">
                      ...
                      <input
                        ...
                        className="flex-1 bg-admin-bg border border-admin-border rounded-2xl px-6 py-3 text-admin-text focus:outline-none focus:border-cyan text-sm outline-none transition-all placeholder:opacity-30"
                      />
                    </div>
                  </div>
                </div>
