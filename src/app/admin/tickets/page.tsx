...
export default function AdminTicketsPage() {
  ...
  const getTypeStyle = (type: string) => {
    switch(type) {
      case 'VIP': return 'text-magenta font-black glow-magenta';
      case 'GA': return 'text-cyan font-bold glow-cyan';
    ...
    <div className="space-y-8 animate-in fade-in zoom-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-admin-text-muted text-sm mt-1">Quản lý danh sách người mua vé, trạng thái thanh toán và thông tin khách hàng</p>
        </div>
        <button 
          onClick={() => handleOpenModal()}
          className="px-6 py-3 bg-cyan hover:bg-cyan/80 text-midnight font-bold rounded-xl flex items-center gap-2 transition-all glow-cyan"
        >
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <div className="relative flex-1 md:flex-none">
              ...
              <select 
                ...
                className="w-full md:w-40 bg-admin-bg border border-admin-border rounded-xl pl-10 pr-4 py-3 text-xs font-bold text-admin-text-muted hover:text-admin-text hover:bg-admin-bg/20 transition-all appearance-none cursor-pointer outline-none focus:border-cyan"
              >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-admin-bg/10 text-admin-text-muted text-xs uppercase tracking-wider">
                <th className="p-4 font-bold border-b border-admin-border whitespace-nowrap">Mã Booking</th>
                ...
                <th className="p-4 font-bold border-b border-admin-border">Số Lượng</th>
                <th className="p-4 font-bold border-b border-admin-border">Tổng Tiền</th>
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={7} className="p-12 text-center text-admin-text-muted">
                  ...
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        <QrCode className="w-4 h-4 text-admin-border group-hover:text-cyan transition-colors" />
                        <span className="font-mono text-xs font-bold text-admin-text">{booking.bookingCode}</span>
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="font-bold text-admin-text text-sm">{booking.name}</div>
                      <div className="text-[10px] text-admin-text-muted mt-0.5">{booking.email}</div>
                    </td>
                    <td className="p-4">
                      <div className={`text-xs ${getTypeStyle(booking.ticketType)}`}>{booking.ticketType}</div>
                    </td>
                    <td className="p-4 text-center font-bold text-admin-text">
                      {booking.quantity}
                    </td>
                    <td className="p-4 font-black text-cyan text-xs">
                      {(booking.totalPrice || 0).toLocaleString()} VNĐ
                    </td>
                    <td className="p-4 hidden sm:table-cell text-xs text-admin-text-muted">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          ...
                          className="p-1.5 rounded-lg bg-admin-border/20 text-cyan border border-cyan/0 hover:border-cyan/50 hover:bg-cyan/10 transition-all" 
                          title="Chỉnh sửa"
                        >
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="relative w-full max-w-2xl bg-admin-panel border border-admin-border rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              <div className="flex justify-between items-center p-6 border-b border-admin-border bg-admin-bg/10 shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-display font-bold text-admin-text uppercase tracking-wider">{editMode ? 'Chỉnh Sửa Vé' : 'Tạo Vé Mới'}</h3>
                </div>
              ...
              <form onSubmit={handleSave} className="flex flex-col flex-1 overflow-hidden">
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <select 
                        ...
                        className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-admin-text focus:outline-none focus:border-cyan appearance-none transition-all"
                      >
                        <option value="GA">General Admission</option>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest block mb-2">Số lượng vé *</label>
                      <input 
                        type="number" 
                        min="1"
                        required
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="text-[10px] font-bold text-admin-text-muted uppercase tracking-widest block mb-2">Tổng Tiền (VNĐ)</label>
                      <input 
                        type="number" 
                        readOnly
                        value={selectedBooking?.totalPrice || 0}
                        className="w-full bg-admin-bg border border-admin-border rounded-xl px-4 py-3 text-cyan font-bold focus:outline-none focus:border-cyan cursor-not-allowed opacity-80" 
                      />
                    </div>
                  </div>

                  {editMode && selectedBooking?.bookingCode && (
                    <div className="p-4 border border-cyan/30 bg-cyan/5 rounded-xl flex items-start gap-3">
                      <QrCode className="w-6 h-6 text-cyan shrink-0 mt-1" />
                      <div>
                        <div className="text-sm font-bold text-admin-text">Mã Booking: {selectedBooking.bookingCode}</div>
                        <div className="text-xs text-admin-text-muted mt-1">Dữ liệu được quản lý trực tiếp từ cơ sở dữ liệu. Mọi thay đổi sẽ được cập nhật ngay lập tức cho khách hàng.</div>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <AnimatePresence>
        {isSuccessModalOpen && lastCreatedBooking && (
          <div className="fixed inset-0 z-[600] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-admin-bg/95 backdrop-blur-xl" />
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-[320px] bg-admin-panel border border-cyan/30 rounded-[1.5rem] shadow-[0_0_50px_rgba(0,255,255,0.2)] overflow-hidden print:bg-white print:border-none print:shadow-none print:rounded-none">
              <div className="p-5 flex flex-col items-center">
                <div className="w-10 h-10 bg-cyan/10 rounded-full flex items-center justify-center mb-3 border border-cyan/30 print:hidden">
                  <CheckCircle2 className="w-5 h-5 text-cyan" />
                </div>
                
                <h3 className="text-lg font-display font-black text-admin-text uppercase tracking-widest mb-0.5 print:text-black print:text-base text-center">Phát Hành Thành Công</h3>
                <p className="text-admin-text-muted text-[9px] mb-4 print:hidden text-center max-w-[180px]">Đã lưu vào hệ thống. Hãy in mã QR này cho khách hàng.</p>
                ...
                <div id="qr-print-area" className="bg-[#FFFFFF] p-4 rounded-xl mb-4 shadow-inner flex flex-col items-center print:p-0 print:shadow-none print:m-0 print:mb-0">
                  ...
                  <div className="mt-3 text-center">
                    <div className="text-[7px] uppercase tracking-[0.3em] font-black text-midnight/30 mb-0.5">Mã Booking</div>
                    <div className="text-base font-mono font-black text-midnight">{lastCreatedBooking.bookingCode}</div>
                  </div>
                </div>

                {/* Thông tin vé */}
                <div className="w-full space-y-2 mb-4 print:text-black px-1">
                  <div className="flex justify-between items-center border-b border-admin-border/10 pb-1.5 print:border-black">
                    <span className="text-[8px] text-admin-text-muted uppercase font-bold print:text-gray-600">Khách Hàng</span>
                    <span className="text-[10px] font-bold text-admin-text print:text-black">{lastCreatedBooking.name}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-admin-border/10 pb-1.5 print:border-black">
                    <span className="text-[8px] text-admin-text-muted uppercase font-bold print:text-gray-600">SĐT</span>
                    <span className="text-[10px] font-bold text-admin-text print:text-black">{lastCreatedBooking.phone || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-admin-border/10 pb-1.5 print:border-black">
                    <span className="text-[8px] text-admin-text-muted uppercase font-bold print:text-gray-600">Hạng Vé</span>
                    <span className={`text-[10px] font-black ${lastCreatedBooking.ticketType === 'VIP' ? 'text-magenta' : 'text-cyan'} print:text-black`}>{lastCreatedBooking.ticketType} TICKET</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-admin-border/10 pb-1.5 print:border-black">
                    <span className="text-[8px] text-admin-text-muted uppercase font-bold print:text-gray-600">Số Lượng</span>
                    <span className="text-[10px] font-bold text-admin-text print:text-black">{lastCreatedBooking.quantity} VÉ</span>
                  </div>
                  <div className="flex justify-between items-center pb-1">
                    <span className="text-[8px] text-admin-text-muted uppercase font-bold print:text-gray-600">Tổng Tiền</span>
                    <span className="text-[10px] font-black text-cyan print:text-black">{(lastCreatedBooking.totalPrice || 0).toLocaleString()} VNĐ</span>
                  </div>
                </div>
                ...
                <div className="flex gap-2 w-full print:hidden">
                  ...
                  <button 
                    ...
                    className="flex-1 py-2.5 rounded-lg bg-cyan text-midnight text-[10px] font-black hover:bg-cyan/80 transition-all flex items-center justify-center gap-2 glow-cyan"
                  >
                ...
                <button 
                  ...
                  className="mt-4 text-[9px] uppercase font-bold text-admin-text-muted hover:text-admin-text transition-colors print:hidden"
                >
                  Đóng & Hoàn Tất
                </button>
              </div>
