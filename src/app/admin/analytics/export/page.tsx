  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-silver/40 font-mono text-sm uppercase tracking-[0.4em]">Integrated Data Export Protocol // v2.0</p>
      </div>

      {/* Main Export Card */}
      <div className="glass-card p-12 rounded-[3.5rem] bg-[#0D0716]/60 border border-white/5 relative overflow-hidden shadow-2xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            ...
            <ul className="space-y-3">
              {modules.map((m, i) => (
                <li key={i} className="flex items-center gap-3 text-xs font-mono uppercase tracking-widest text-silver/40">
            ...
            <div className="pt-6">
              <button 
                ...
                className={`w-full py-6 rounded-2xl flex items-center justify-center gap-4 text-sm font-black uppercase tracking-[0.2em] transition-all relative overflow-hidden group ${
                  isExporting ? 'bg-white/5 text-silver/20' : 'bg-gradient-primary text-white hover:glow-magenta'
                }`}
              >
                {isExporting ? (
                  <>
          ...
          <div className="hidden lg:flex flex-col items-center justify-center space-y-6 relative">
             <div className="relative">
                ...
                <div className="w-56 h-72 border-2 border-white/10 rounded-[2.5rem] bg-black/40 relative z-10 flex flex-col items-center justify-center p-8 text-center group hover:border-cyan/50 transition-colors">
                   <div className="w-20 h-20 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mb-6 text-cyan group-hover:bg-cyan/20 transition-all">
                      <FileDown size={40} />
                   </div>
                   <div className="text-[10px] font-mono text-silver/30 font-bold uppercase tracking-tighter mb-2">OUTPUT FORMAT</div>
                   <div className="text-3xl font-black text-white italic">.XLSX</div>
             ...
             <p className="text-[10px] font-mono text-silver/20 uppercase tracking-[0.5em]">Heritage Data Node // Secure Channel</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
         <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/5 flex gap-5 items-start">
            <ShieldCheck className="text-cyan shrink-0 mt-1" />
            <div>
               <h4 className="text-sm font-bold text-white uppercase mb-2">Bảo mật dữ liệu</h4>
               <p className="text-xs text-silver/40 leading-relaxed font-mono">Báo cáo chứa các thông tin nhạy cảm về doanh thu và thông tin cá nhân khách hàng. Vui lòng chỉ chia sẻ với những người có thẩm quyền.</p>
            </div>
         </div>
         <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/5 flex gap-5 items-start">
            <BarChart3 className="text-magenta shrink-0 mt-1" />
            <div>
               <h4 className="text-sm font-bold text-white uppercase mb-2">Định dạng báo cáo</h4>
               <p className="text-xs text-silver/40 leading-relaxed font-mono">File Excel đã được định dạng sẵn kiểu Bảng (Table). Bạn có thể dễ dàng sử dụng bộ lọc (Filter) trong Excel để truy vấn dữ liệu nhanh.</p>
            </div>
         </div>
      </div>
    </div>
