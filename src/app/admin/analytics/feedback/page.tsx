  ...
  if (loading && !data) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-admin-text-muted font-mono text-xs uppercase tracking-[0.2em]">Decrypting feedback clusters...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in zoom-in duration-500 pb-10">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          <h2 className="text-4xl font-display font-black uppercase text-admin-text tracking-tighter flex items-center gap-3">
            <MessageSquare className="w-10 h-10 text-magenta glow-magenta" />
            Đánh giá & Phản hồi
          </h2>
          <p className="text-admin-text-muted text-sm mt-1 uppercase tracking-[0.3em] font-mono">Customer Insights Protocol // v1.2 (UPDATED)</p>
        </div>
        <button 
          onClick={fetchData}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Tổng đánh giá', value: summary?.totalReviews || 0, sub: 'Responses', icon: MessageSquare, color: 'var(--admin-text)' },
          { label: 'Điểm trung bình', value: summary?.avgOverall || 0, sub: 'Stars Out of 5.0', icon: Star, color: 'var(--gold)' },
          { label: 'Báo cáo Spam', value: summary?.spamCount || 0, sub: 'Filtered Bot Submissions', icon: ShieldAlert, color: 'var(--magenta)' },
          { label: 'Đánh giá hợp lệ', value: summary?.validReviews || 0, sub: 'Verified Human Insights', icon: Activity, color: 'var(--cyan)' },
        ].map((s, i) => (
          <motion.div 
            key={i} 
            ...
            className="p-8 rounded-[2rem] bg-admin-panel/80 backdrop-blur-md border border-admin-border/5 relative overflow-hidden group hover:border-admin-text/20 transition-all shadow-2xl"
          >
            <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-all">
              <s.icon className="w-16 h-16" style={{ color: s.color }} />
            </div>
            <p className="text-admin-text-muted text-[10px] font-black uppercase tracking-widest mb-1">{s.label}</p>
            <div className="text-4xl font-display font-black" style={{ color: s.color }}>{s.value}</div>
            <p className="text-[10px] text-admin-text-muted/50 mt-2 font-mono uppercase tracking-tighter">{s.sub}</p>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-12 xl:col-span-8 glass-card p-10 rounded-[3rem] bg-admin-panel/80 border border-admin-border/5 min-h-[500px] flex flex-col shadow-2xl relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-12 relative z-10">
            <h3 className="text-sm font-black uppercase tracking-[0.3em] text-admin-text flex items-center gap-3">
              <div className="w-2 h-2 bg-cyan rounded-full animate-pulse" />
              Chỉ số hài lòng đa chiều
            </h3>
            <div className="flex items-center gap-2 text-[10px] text-admin-text-muted font-mono">
          ...
          <div className="flex-1 min-h-[600px]">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <ChartTooltip 
                  contentStyle={{ backgroundColor: 'var(--admin-panel)', border: '1px solid var(--admin-border)', borderRadius: '12px', fontSize: '12px', fontWeight: 'bold' }}
                  itemStyle={{ color: '#FF0088' }}
                />
              </RadarChart>
            </ResponsiveContainer>
          </div>
          
          <div className="mt-8 flex flex-wrap gap-4 items-center justify-center relative z-10 border-t border-admin-border/10 pt-8">
             <div className="p-4 rounded-2xl bg-magenta/5 border border-magenta/10 flex items-start gap-4 flex-1">
                ...
                <div>
                   <p className="text-[10px] text-admin-text font-black uppercase mb-1">Cảnh báo Spam</p>
                   <p className="text-[9px] text-admin-text-muted font-mono leading-relaxed">Hệ thống lọc bỏ các phản hồi hoàn thành dưới 45 giây để đảm bảo tính xác thực của dữ liệu Radar.</p>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-12 xl:col-span-4 glass-card rounded-[3rem] bg-admin-panel/80 border border-admin-border/5 overflow-hidden shadow-2xl flex flex-col">
          ...
          <div className="p-8 bg-admin-bg/40 border-t border-admin-border/5 text-center">
             <p className="text-[10px] text-admin-text-muted/20 font-mono uppercase tracking-[0.4em]">Neon Heritage Insights Protocol</p>
          </div>
        </div>
      </div>
    </div>
