      ...
      <div className="min-h-screen bg-[#060010] flex items-center justify-center">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-8 h-8 border-4 border-magenta/20 border-b-magenta rounded-full animate-spin-reverse"></div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#060010] text-white pt-32 pb-20 px-6 overflow-hidden relative">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
        <div className="absolute -top-[20%] -left-[10%] w-[60%] h-[60%] bg-royal/10 rounded-full blur-[120px] animate-pulse"></div>
        <div className="absolute -bottom-[20%] -right-[10%] w-[60%] h-[60%] bg-magenta/5 rounded-full blur-[120px] animate-pulse-slow"></div>
      ...
      <div className="container mx-auto max-w-5xl relative z-10">
        <header className="mb-12">
          ...
          >
            <div className="h-px w-12 bg-cyan"></div>
            <span className="text-cyan font-display font-bold uppercase tracking-widest text-xs">Khu vực cá nhân</span>
          </motion.div>
          ...
          <motion.h1 
            ...
            className="text-4xl md:text-6xl font-display font-black uppercase tracking-tighter mb-4"
          >
            Vé của <span className="text-gradient">Bạn</span>
          </motion.h1>
        ...
        {bookings.length === 0 ? (
          ...
          >
            ...
            <Link 
              href="/"
              className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-magenta to-royal rounded-xl font-bold uppercase tracking-widest hover:scale-105 transition-all shadow-[0_0_20px_rgba(255,0,136,0.3)]"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {bookings.map((booking, i) => (
              ...
              >
                {/* Visual Glow Effect */}
                <div className={`absolute -inset-0.5 bg-gradient-to-r ${booking.ticketType === 'VIP' ? 'from-magenta to-royal' : 'from-cyan to-blue-600'} rounded-3xl blur opacity-20 group-hover:opacity-40 transition duration-500`}></div>
                
                <div className="relative h-full flex flex-col bg-midnight/90 border border-white/10 rounded-3xl overflow-hidden backdrop-blur-xl">
                  {/* Ticket Header */}
                  <div className={`p-6 ${booking.ticketType === 'VIP' ? 'bg-magenta/10 border-b border-magenta/20' : 'bg-cyan/10 border-b border-cyan/20'} flex items-center justify-between`}>
                    <div className="flex items-center gap-3">
                      <div className={`p-3 rounded-2xl ${booking.ticketType === 'VIP' ? 'bg-magenta/20' : 'bg-cyan/20'}`}>
                        <Ticket className={`w-6 h-6 ${booking.ticketType === 'VIP' ? 'text-magenta' : 'text-cyan'}`} />
                      </div>
                      <div>
                        <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${booking.ticketType === 'VIP' ? 'text-magenta' : 'text-cyan'}`}>Loại Vé</p>
                  ...
                  <div className="p-6 bg-white/5 border-t border-white/10 flex items-center justify-between mt-auto">
                    <div className="flex items-center gap-2">
                       <div className={`w-2 h-2 rounded-full ${booking.status === 'PAID' || booking.status === 'confirmed' ? 'bg-teal animate-pulse' : 'bg-gold'}`}></div>
                       <span className={`text-[10px] font-black uppercase tracking-widest ${booking.status === 'PAID' || booking.status === 'confirmed' ? 'text-teal' : 'text-gold'}`}>
                         {booking.status === 'PAID' || booking.status === 'confirmed' ? 'Đã Thanh Toán' : 'Đang Chờ'}
                       </span>
                    </div>
                    <button 
                      onClick={() => setSelectedTicket(booking)}
                      className="px-6 py-2.5 bg-white text-[#060010] rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-cyan transition-all flex items-center gap-2"
                    >
        ...
        {selectedTicket && (
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-sm bg-midnight border border-white/10 rounded-[3rem] p-10 text-center"
            >
              <div className="mb-8">
                <h3 className="text-xl font-display font-bold uppercase tracking-widest text-white">QR Code Check-in</h3>
                <p className="text-gray-400 text-xs mt-2 uppercase tracking-widest letter-spacing-2">Mã: {selectedTicket.bookingCode}</p>
              </div>

              <div className="relative aspect-square w-full max-w-[200px] mx-auto p-4 bg-white rounded-[2rem] mb-8">
                <div className="w-full h-full bg-[url('https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=https://neon-heritage.vn/check-in/')] bg-no-repeat bg-center bg-contain opacity-90"></div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-12 h-12 bg-midnight rounded-2xl flex items-center justify-center border border-white/10">
              ...
              <div className="space-y-4 mb-8">
                <p className="text-xs font-bold text-silver uppercase tracking-widest">{selectedTicket.name}</p>
