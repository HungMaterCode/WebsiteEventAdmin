...
export default function HeritageArtSection() {
  const instruments = [
    { name: "Đàn Tính", desc: "Nhạc cụ linh hồn của người Tày, Nùng, Thái. Tiếng đàn thanh cao hòa quyện cùng âm hưởng điện tử hiện đại.", icon: <Music className="w-8 h-8" /> },
    { name: "Trống Hội", desc: "Âm vang hào hùng từ những chiếc trống truyền thống, tạo nên nhịp đập mạnh mẽ cho dòng chảy di sản.", icon: <Sparkles className="w-8 h-8" /> },
    { name: "Khèn Mông", desc: "Giai điệu phóng khoáng của núi rừng Tây Bắc được tái hiện qua lăng kính âm nhạc đương đại.", icon: <Music className="w-8 h-8" /> },
  ];
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-16">
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="w-full lg:w-1/2">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-display font-black uppercase tracking-wider mb-6 md:mb-8">
              <span className="text-silver">Nghệ Thuật </span><span className="text-gradient block">Di Sản Cyber</span>
            </h2>
            <p className="text-gray-400 text-base md:text-lg leading-relaxed mb-8 md:mb-10">Tại Neon Heritage Festival, chúng tôi không chỉ trình diễn âm nhạc. Chúng tôi kể câu chuyện về sự giao thoa giữa quá khứ và tương lai.</p>
            <div className="space-y-4 md:space-y-6">
              {instruments.map((item, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="flex flex-col sm:flex-row gap-4 sm:gap-6 p-5 md:p-6 rounded-2xl bg-royal/5 border border-royal/20 hover:border-gold/40 transition-colors group">
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl bg-gold/10 flex items-center justify-center text-gold group-hover:bg-gold group-hover:text-midnight transition-all shrink-0">{item.icon}</div>
                  <div>
            ...
            <div className="relative aspect-square max-w-[280px] sm:max-w-md mx-auto">
              ...
              <div className="relative z-10 w-full h-full border-2 border-gold/30 rounded-full p-4 md:p-8 flex items-center justify-center">
                ...
                <div className="absolute inset-0 flex items-center justify-center p-6 md:p-12">
                  <img src="https://picsum.photos/seed/heritage-art/800/800" alt="Heritage Art" className="w-full h-full object-cover rounded-full border-2 md:border-4 border-midnight shadow-[0_0_30px_rgba(230,199,83,0.3)]" referrerPolicy="no-referrer" />
                </div>
              </div>
            </div>
          </motion.div>
